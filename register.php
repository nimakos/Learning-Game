...
if(!empty($_POST["register-user"])) 
{
	...
	if(!isset($error_message)) //εαν δεν εχει τιμη η μεταβλητη error
	{
		...
		/* Επικύρωση Αριθμού Μητρώου */
		
		if(strlen($_POST["userName"])!==9 )
		{
			
			$error_message = "Οι χαρακτήρες του αριθμού μητρώου πρέπει να είναι 9";
		}
		
		if(0!==strpos(($_POST["userName"]), 'mppl' ) )
		{
			$error_message = "Ο αριθμός μητρώου πρέπει να ξεκινάει με ΄mppl΄";
		}
		
		$startposition=4;
		while($startposition < 9)
		{
		if(!is_numeric(substr($_POST["userName"],$startposition)))
		{
			
				$error_message = "Ο αριθμός μητρώου θα πρέπει να περιεχει και αριθμούς";
